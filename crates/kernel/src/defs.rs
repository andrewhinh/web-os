...
macro_rules! array {
    [$e:expr; $count:expr] => {{
        ...
        let mut arr_in: [MaybeUninit<_>; $count] = unsafe { MaybeUninit::uninit().assume_init() };
        let mut idx = 0;
        while idx < $count {
        ...
        unsafe { ManuallyDrop::into_inner(_transmuter { arr_in: ManuallyDrop::new(arr_in) }.arr_out) }
    }};
}
...
// - A reference to T must be able to coexist with a byte slice reference to the
//   same memory.
...
// # Safety
// - The memory layout of T must be fixed and all bytes of T must be valid.
// - It must be safe to interpret a properly aligned byte slice of the correct
//   length as an instance of T.
// - A reference to T must be able to coexist with a byte slice reference to the
//   same memory.
